<script lang="ts">
	...
	import { deleteOrganizationMutationQuery } from '$lib/features/organizations/mutations';
	import { toast } from 'svelte-sonner';
	import { goto } from '$app/navigation';

	const organizationsQuery = createOrganizationsListQuery();
	const deleteMutation = deleteOrganizationMutationQuery({
		onSuccess: () => {
	...
	function handleDeleteOrganization() {
		if (!currentOrg || !canDeleteOrg) return;

		deleteMutation.mutate(currentOrg.id);
