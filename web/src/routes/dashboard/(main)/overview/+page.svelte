<script lang="ts">
	...
	import { IconWorldLatitude, IconDropCircle } from '@tabler/icons-svelte';
	import * as Empty from '$lib/components/ui/empty/index.js';
	import Timeline from '$lib/features/activities/components/timeline.svelte';
...
<div class="min-h-screen px-6">
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		<div class="lg:col-span-2 space-y-6">
			<div>
				...
				{:else if projects.data && projects.data.length === 0}
					<Empty.Root class="border border-border">
						...
						<Empty.Content>
							<div>
								<Button href="/dashboard/projects">Create Project</Button>
							</div>
						</Empty.Content>
					</Empty.Root>
				{:else if projects.data}
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						{#each projects.data as project (project.id)}
							<a
								class="bg-card border border-border rounded-lg min-h-38 p-4 hover:bg-card-hover transition-colors cursor-pointer flex flex-col justify-between"
								href={`/dashboard/project/${project.id}`}
							>
								<div class="flex items-center justify-between mb-8">
									<div class="flex items-center space-x-2">
										<IconWorldLatitude class="size-5 text-muted-foreground" />
										<span class="text-sm capitalize">{project.name}</span>
									</div>
								</div>
			...
			<!-- 				<div class="text-center text-muted-foreground text-sm py-4">No servers configured</div> -->
