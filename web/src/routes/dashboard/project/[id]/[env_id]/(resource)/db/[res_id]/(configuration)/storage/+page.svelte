<script lang="ts">
	...
	import { disksApi, type CreateDiskRequest } from '$lib/features/disks/api';
	import { createDisksListQuery } from '$lib/features/disks/queries';
	import {
		createDiskMutationQuery,
		deleteDiskMutationQuery,
		detachDiskMutationQuery
	} from '$lib/features/disks/mutations';

	const projectId = page.params.id!;
	const resId = page.params.res_id!;
	const queryClient = useQueryClient();

	const databaseQuery = createDatabaseFetchQuery(projectId, resId);
	const disksQuery = createDisksListQuery(projectId);

	const deleteMutation = deleteDiskMutationQuery(projectId);
	const detachMutation = detachDiskMutationQuery(projectId);
	const createMutation_ = createDiskMutationQuery(projectId, {
		onSuccess: () => {
