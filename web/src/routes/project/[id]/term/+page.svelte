<script>
	...
	let terminalHistory = $state([
		{ type: 'system', content: 'Welcome to Sevalla Web Terminal' },
		{ type: 'system', content: 'Type "help" for available commands' },
		{ type: 'prompt', content: 'user@focalpoint-dashboard:~$ ' }
	]);
	...
	function connectTerminal() {
		...
		terminalHistory = [
			...terminalHistory,
			{ type: 'system', content: 'Connected to focalpoint-dashboard-prod' },
			{ type: 'prompt', content: 'root@focalpoint-dashboard:~$ ' }
		];
	}
	...
	function executeCommand() {
		...
		terminalHistory = [
			...
			{
				type: 'command',
				content: `${isConnected ? 'root' : 'user'}@focalpoint-dashboard:~$ ${currentCommand}`
			}
		];
  ...
  clear       - Clear terminal`;
				break;
			case 'ls':
				response = 'app.js  package.json  node_modules  public  src  .env  .git';
				break;
			case 'pwd':
				response = '/home/app';
				break;
...
app        123  2.1  5.4 932844 55432 ?        Sl   14:30   0:45 node app.js
app        456  0.0  0.2  12345  2048 ?        S    14:35   0:00 npm run build`;
				break;
			case 'df -h':
...
tmpfs           2.0G     0  2.0G   0% /dev/shm`;
			...
			case 'clear':
				terminalHistory = [
					...
					{ type: 'prompt', content: `${isConnected ? 'root' : 'user'}@focalpoint-dashboard:~$ ` }
				];
				currentCommand = '';
				return;
			default:
				if (currentCommand.startsWith('tail -f')) {
					response = `Following ${currentCommand.split(' ')[2] || '/var/log/app.log'}...
2024-01-31 14:30:15 [INFO] Server started on port 3000
2024-01-31 14:30:16 [INFO] Database connection established
2024-01-31 14:30:17 [INFO] Application ready to accept connections`;
				} else {
...
<svelte:head>
	<title>Web Terminal - {project.name}</title>
</svelte:head>
...
<div class="flex-1 p-6">
	...
	<div class="flex items-center justify-between mb-6">
		<div class="flex items-center space-x-4">
			<div>
				<h1 class="text-2xl font-semibold text-gray-900">Web Terminal</h1>
				<p class="text-sm text-gray-500 mt-1">
					Access your server directly through the web terminal for {project.name}.
				</p>
			</div>
		</div>
				...
				<div class="p-4 space-y-1">
					...
					<div class="flex items-center">
						<span class="text-green-400"
							>{isConnected ? 'root' : 'user'}@focalpoint-dashboard:~$
						</span>
						<input
