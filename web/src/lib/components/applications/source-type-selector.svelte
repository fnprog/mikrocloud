<script lang="ts">
	...
	interface Props {
		selected: 'git' | 'docker' | 'registry' | 'zip';
		onSelect: (type: 'git' | 'docker' | 'registry' | 'zip') => void;
	}
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
	...
	<Card
		class={cn(
			'cursor-pointer transition-all hover:border-primary/50',
			selected === 'registry' && 'border-primary border-2'
		)}
	...
	>
		<CardContent class="flex flex-col items-center justify-center py-8 space-y-4">
			<div
				class={cn(
					'w-12 h-12 rounded-full flex items-center justify-center',
					selected === 'registry' ? 'bg-primary text-primary-foreground' : 'bg-muted'
				)}
			>
				<Container class="h-6 w-6" />
			</div>
			<div class="text-center">
				<h3 class="font-semibold text-lg">Registry image</h3>
				<p class="text-sm text-muted-foreground mt-1">Pull from Docker Hub or registry</p>
			</div>
		</CardContent>
	</Card>
